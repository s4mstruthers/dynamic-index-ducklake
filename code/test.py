...
from helper_functions import connect_ducklake, tokenize
...
def run_tests():
    ...
    try:
        ...
        # ---------- T6: modify doc2 ----------
        modify_doc(con, doc2_id, new2) # type: ignore
        ...
        stray = 0
        if doc1_id is not None:
            stray += con.execute("SELECT COUNT(*) FROM my_ducklake.docs WHERE docid = ?", (doc1_id,)).fetchone()[0] # type: ignore
            stray += con.execute("SELECT COUNT(*) FROM my_ducklake.data WHERE docid = ?", (doc1_id,)).fetchone()[0] # type: ignore
            stray += con.execute("SELECT COUNT(*) FROM my_ducklake.postings WHERE docid = ?", (doc1_id,)).fetchone()[0] # type: ignore
        if doc2_id is not None:
            stray += con.execute("SELECT COUNT(*) FROM my_ducklake.docs WHERE docid = ?", (doc2_id,)).fetchone()[0] # type: ignore
            stray += con.execute("SELECT COUNT(*) FROM my_ducklake.data WHERE docid = ?", (doc2_id,)).fetchone()[0] # type: ignore
            stray += con.execute("SELECT COUNT(*) FROM my_ducklake.postings WHERE docid = ?", (doc2_id,)).fetchone()[0] # type: ignore
